  ...
  return (
    ...
    >
      ...
      <div className="aspect-square relative overflow-hidden">
        ...
        {/* Overlay */}
        <div className={`absolute inset-0 bg-black/60 flex items-center justify-center transition-opacity duration-300 ${isHovered || isPlaying ? 'opacity-100' : 'opacity-0'}`}>
          <button 
        ...
        <div className="flex items-center justify-between gap-3 pt-4 border-t border-white/5">
          <div className="flex gap-2">
            {beat.tags.slice(0, 2).map((tag) => (
              <span key={tag} className="px-2 py-1 rounded-md bg-white/5 text-[10px] uppercase tracking-wider text-neutral-400 border border-white/5">
                {tag}
              </span>
            ))}
          </div>
          
          <Button size="sm" className="rounded-full bg-[hsl(var(--gold))] text-black hover:bg-[hsl(var(--gold))/90] font-bold text-xs uppercase px-4">
