        ...
        body: JSON.stringify({
          orderData: {
            ...
            license_url: orderData.items.some((item: any) =>
              ...
              : orderData.customLicenseId
                ? `${window.location.origin}/license/custom/${orderData.customLicenseId}`
...
export default function Shop() {
  ...
  const handlePaymentSuccess = async (
    ...
    try {
      ...
      if (user) {
        ...
        // Create custom licenses for beat purchases
        let customLicenseId = null;
        const beatItems = validItems.filter(
          (item) =>
            !item.id.startsWith("subscription-") &&
            !item.id.startsWith("license-") &&
        ...
        if (beatItems.length > 0 && orderData && customArtistName) {
          ...
          if (!licenseError && customLicense) {
            customLicenseId = customLicense.id;
          }
        }
  ...
  const handleStripeSuccess = async (sessionId: string) => {
    ...
    try {
      ...
      // Hydrate items with full metadata from local beats (covers, correct titles)
      // This fixes issues where localStorage might have stale or incomplete data
      const hydratedItems = cartItems.map((item: any) => {
        const localBeat = beats.find((b) => String(b.id) === String(item.id));
        if (localBeat) {
          return {
            ...item,
            title: localBeat.title,
            artist: localBeat.artist,
            cover: localBeat.cover,
            // Keep price/quantity from cart as they might be dynamic/discounted
          };
        }
        return item;
      });
      ...
      if (orderError) {
        ...
        if (orderError.code === "23505") {
          ...
          if (hasIncompleteItems && itemsToSave.length > 0) {
            console.log(
      ...
      if (orderData) {
        // Create custom licenses for beat purchases
        let customLicenseId = null;
        const beatItems = cartItems.filter(
          (item: any) =>
            !item.id.startsWith("subscription-") &&
            !item.id.startsWith("license-") &&
            item.id !== "royalty-token"
